...
def _move_is_possible(move, game_state, move_history):
    ...
    if is_pawn_cut:
        updated_game_state[start_row][end_col] = '0'
...
def _get_pawn_moves(pos, game_state, move_history):
    ...
    if my_color == 'b':
        ...
        if len(move_history):
            ...
            if (row == 3 and (col - 1) >= 0 and
                    game_state[row][col - 1] == 'p' and
                    move_history[-1][0] == (row - 2, col - 1) and
                    move_history[-1][1] == (row, col - 1)):
                moves.append((row - 1, col - 1))
            # E4F3 if white last was pawn F2F4
            if (row == 3 and (col + 1) < 8 and
                    game_state[row][col + 1] == 'p' and
                    move_history[-1][0] == (row - 2, col + 1) and
                    move_history[-1][1] == (row, col + 1)):
                moves.append((row - 1, col + 1))
        ...
        if len(move_history) and row < 7:
            if (row == 4 and (col - 1) >= 0 and
                    game_state[row][col - 1] == 'P' and
                    move_history[-1][0] == (row + 2, col - 1) and
                    move_history[-1][1] == (row, col - 1)):
                moves.append((row + 1, col - 1))
            # E4F5
            if (row == 4 and (col + 1) < 8 and
                    game_state[row][col + 1] == 'P' and
                    move_history[-1][0] == (row + 2, col + 1) and
                    move_history[-1][1] == (row, col + 1)):
                moves.append((row + 1, col + 1))
...
def validate_moves_history(portable_moves_history):
    ...
    exodus = portable_moves_history[-1]
    ...
    return algebraic_notation, pawn_promotion, exodus
...
def check_enpassant(row, col, game_state, move_history):
    my_color = 'b' if len(move_history) % 2 else 'w'
    if my_color == 'w':
        if (row == 4 and (col - 1) >= 0 and
                game_state[row][col - 1] == 'P' and
...
def repetition_three_times(move, current_state, moves_history, dict_states, copy_dict, w_pos, b_pos, number):
    enpassant = []
    castling = ''
    direction = int
    lens_state = len(current_state)
    white_castling = _get_possible_castlings(w_pos['k'][0], current_state, moves_history)
    black_castling = _get_possible_castlings(b_pos['K'][0], current_state, moves_history)
    # w_move = _get_available_moves(w_pos, current_state, moves_history)
    # b_move = _get_available_moves(b_pos, current_state, moves_history)
    # print('current_string', string)
    new_current = ''

    for b_pawn_pos in b_pos['P']:
        row, col = b_pawn_pos
        if check_enpassant(row, col, current_state, moves_history):
            enpassant.append(b_pawn_pos)
    for w_pawn_pos in w_pos['p']:
        row, col = w_pawn_pos
        if check_enpassant(row, col, current_state, moves_history):
            enpassant.append(w_pawn_pos)

    # It is necessary to check the order of the move according to
    # the current history of moves.
    # If the number is even according order move - white in code - 0
    direction = 1 if lens_state % 2 else 0

    for pos in white_castling:
        if pos[1] == 5:
            castling += 'q'
        else:
            castling += 'k'

    for pos in black_castling:
        if pos[1] == 5:
            castling += 'Q'
        else:
            castling += 'K'

    string = str(direction) + str(enpassant) + castling + str(current_state)
    print('string', string)

    if string in dict_states:
        dict_states[string] += 1
        copy_dict[string].append(number+1)
        if dict_states[string] == 3:
            print("work")
            return 3
    ...
    for row in current_state:
        ...
        new_current += row
    print('move', move)
    print(f'copy_dic_number = {copy_dict[string]}')
    print(f'repeat: {dict_states[string]} state:\n{new_current}\n')

    return copy_dict, dict_states


def not_enough_pieces(board_state):
    w_pos, b_pos = _get_figures_positions(board_state)
    black_fig, b_bishop_color = [], None
    white_fig, w_bishop_color = [], None
    check_white = True
    check_black = True
    for fig, pos in w_pos.items():
        if (fig == 'n' and len(pos) == 1) or (fig == 'b' and len(pos)) == 1 or fig == 'k':
            white_fig.append(fig)
            if fig == 'b':
                row, col = w_pos[fig][0]
                white_bishop_color = 'b' if (row + col) % 2 else 'w'
        else:
            check_white = False
            break
    for fig, pos in b_pos.items():
        if (fig == 'N' and len(pos) == 1) or (fig == 'B' and len(pos)) == 1 or fig == 'K':
            black_fig.append(fig)
            if fig == 'b':
                row, col = b_pos[fig][0]
                black_bishop_color = 'b' if (row + col) % 2 else 'w'
        else:
            check_black = False
            break
    white_fig = list(map(lambda x: x.upper, white_fig))
    if check_white and check_black:
        if abs(len(white_fig) - len(black_fig)) == 1:
            return True
        elif white_fig == black_fig and b_bishop_color == w_bishop_color:
            return True
    else:
        return False


def to_standard_notation(moves_history, pawn_promotion, exodus=None):
    current_state = copy.deepcopy(default_state)
    last_state = copy.deepcopy(default_state)
    standard_notation = []
    ...
    for index, (start_pos, end_pos) in enumerate(moves_history):
            ...
            if current_state[end_row][end_col] == '0':
                ...
                if len(commons_moves_algebraic) == 0 and fig.lower() != 'p':
                    ...
                    if check_row and not check_col:
                        ...
                        move_in_standard = f'{top_labels[start_col]}{start_row + 1}x{top_labels[end_col]}{end_row + 1}'
        ...
        current_state, event, draw_move_3, draw_move_50, new_available_moves = _event_check(index,
                                                                                            ...
                                                                                            draw_move_3,
                                                                                            check_promo)
        # if _check_remove_piece((start_pos, end_pos), last_state) or is_pawn_cut\
        #         or not check_promo\
        #         or event in {'O-O', 'O-O-O'}:
        #     copy_dict, dict_states = repetition_three_times(move_in_standard, current_state,
        #                                                     current_move_history,{},
        #                                                     {}, w_pos, b_pos, index)
        #
        # else:
        #     result_rep_func = repetition_three_times(move_in_standard, current_state, current_move_history, dict_states,
        #                                                     copy_dict, w_pos, b_pos, index)
        #
        #     if result_rep_func == 3:
        #         repetition = True
        #     else:
        #         copy_dict, dict_states = result_rep_func


        last_state = copy.deepcopy(current_state)
        check_enough_piece = not_enough_pieces(current_state)
        fig = fig if fig.lower() != 'p' else ''
        number_state[index+1] = current_state

        if event in {'O-O', 'O-O-O'}:
            move_in_standard = event
        else:
            if ((index + 1) in pawn_promotion and
                    length_history == index + 1 and
                    '1/2' in exodus and '1/2' not in event):

                fig = f'={pawn_promotion[index + 1].upper()}'
                move_in_standard = move_in_standard + fig.upper() + event + ' 1/2' if color == 'w' \
                    else fig.upper() + move_in_standard + '' + event

            elif (index + 1) in pawn_promotion:

                fig = f'={pawn_promotion[index + 1].upper()}'
                if '1/2' not in event:
                    move_in_standard = move_in_standard + fig + event
                else:
                    move_in_standard = move_in_standard + fig + ' 1/2' if color == 'w' \
                    else move_in_standard + fig

            elif check_enough_piece:

                move_in_standard = fig.upper() + move_in_standard + event + ' 1/2' if color == 'w' \
                             else fig.upper() + move_in_standard + '' + event

            elif length_history == index + 1 and '1/2' in exodus and '1/2' not in event:

                move_in_standard = (fig.upper() + move_in_standard + event) + ' 1/2' if color == 'w' \
                    else fig.upper() + move_in_standard + '' + event

            elif not any(x in event for x in {'O-O', '1/2'}):

                move_in_standard = fig.upper() + move_in_standard + event if color == 'w' \
                    else fig.upper() + move_in_standard + event

            elif '1/2' in event and color == 'w':

                move_in_standard = fig.upper() + move_in_standard + ' ' + '1/2'

            elif '1/2' in event and color == 'b':
    ...
    * performs moves and updates view state.
    ...
    def perform_move(self, start_pos, end_pos, fig=None):
        if fig is None:
            ...
            start_row, start_col = start_pos
            end_row, end_col = end_pos
            w_pos, b_pos = _get_figures_positions(self._board_state)
            ...
            self._dict_states = repetition_three_times(self._board_state,
                                                       self._moves_history,
                                                       self._dict_states,
                                                       w_pos, b_pos)
